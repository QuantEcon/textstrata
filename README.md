...
- Blame credits the last toucher: a one-character fix claims the whole line, so human shares are an **upper bound** at line granularity. Churn is therefore also reported in changed characters, where a one-character fix counts as one character.
